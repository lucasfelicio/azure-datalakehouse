...
resource "azurerm_databricks_workspace" "databricks_prod" {
  name = "adb-${local.prod_fixed}"
  resource_group_name = azurerm_resource_group.rg_prod.name
  location = var.region
  sku = "premium"
  managed_resource_group_name = "rg-adb-${local.prod_fixed}"  
}

resource "azurerm_databricks_workspace" "databricks_dev" {
  name = "adb-${local.dev_fixed}"
  resource_group_name = azurerm_resource_group.rg_dev.name
  location = var.region
  sku = "premium"
  managed_resource_group_name = "rg-adb-${local.dev_fixed}"  
}